...
num_test_1c=floor(size(predict_label_out,1)/num_class);

confusion_matrix=size(num_class,num_class);

for ci=1:num_class
    for cj=1:num_class
       confusion_matrix(ci,cj)=size(find(predict_label_out((ci-1)*num_test_1c+1:ci*num_test_1c,:)==cj),1)/num_test_1c;
    end
end
